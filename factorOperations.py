...
def joinFactors(factors):
    ...
    for factor in factors:
        for uVar in factor.unconditionedVariables():
            joinedUnconditionedVariables.add(uVar)
        for cVar in factor.conditionedVariables():
            joinedConditionedVariables.add(cVar)
            
    joinedConditionedVariables = {var for var in joinedConditionedVariables if var not in joinedUnconditionedVariables}
    ...
    for joinedAssignment in joinedFactor.getAllPossibleAssignmentDicts():
        ...
        for factor in factors:
            factorAssignmentProduct *= Factor.getProbability(factor, joinedAssignment)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        "*** YOUR CODE HERE ***"
        temp = factor.unconditionedVariables()
        temp.remove(eliminationVariable)
        reducedFactor = Factor(temp, factor.conditionedVariables(), factor.variableDomainsDict())
        ...
        for reducedAssignment in reducedFactor.getAllPossibleAssignmentDicts():
            ...
            for elimVarVal in factor.variableDomainsDict()[eliminationVariable]:
                reducedAssignment[eliminationVariable] = elimVarVal
                prob += factor.getProbability(reducedAssignment)
...
def normalize(factor):
    ...
    "*** YOUR CODE HERE ***"
    newUVars = set() # unconditioned
    newCVars = set() # conditioned
    for variable in factor.conditionedVariables():
        newCVars.add(variable)
    for variable in factor.unconditionedVariables():
        if len(factor.variableDomainsDict()[variable]) == 1:
            newCVars.add(variable)
        else:
            newUVars.add(variable)

    normalizedFactor = Factor(newUVars, newCVars, factor.variableDomainsDict())
    
    probS = sum([factor.getProbability(assignment) for assignment in factor.getAllPossibleAssignmentDicts()])

    for assignment in normalizedFactor.getAllPossibleAssignmentDicts():
        normalizedFactor.setProbability(assignment, factor.getProbability(assignment) / probS)

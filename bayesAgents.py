...
def constructBayesNet(gameState):
    ...
    variableDomainsDict[FOOD_HOUSE_VAR] = variableDomainsDict[GHOST_HOUSE_VAR]
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsFactor = bn.Factor([OBS_VAR_TEMPLATE % obsPos], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                probRed = 0
                probBlue = 0
                probNone = 0

                food = assignment[FOOD_HOUSE_VAR] == getHouseValCorrespToPos(obsPos[0], obsPos[1])
                ghost = assignment[GHOST_HOUSE_VAR] == getHouseValCorrespToPos(obsPos[0], obsPos[1])
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    dictList = list(factor.getAllPossibleAssignmentDicts())
    def getProb(x):
        return factor.getProbability(x)
    dictList.sort(key = getProb, reverse = True)
    return dictList[0]
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        p_left = 0
        p_right = 0
        for assignment in inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder).getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                p_right = inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder).getProbability(assignment)
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                p_left = inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder).getProbability(assignment)
            if p_left != 0 and p_right != 0:
                break

        return p_left * WON_GAME_REWARD + (1  - p_left) * GHOST_COLLISION_REWARD, p_right * WON_GAME_REWARD + (1 - p_right) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        return sum([prob * max(self.computeEnterValues(ev, enterEliminationOrder))
                    for (prob, ev) in self.getExplorationProbsAndOutcomes(evidence)])
